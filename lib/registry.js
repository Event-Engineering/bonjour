...
export default class Registry {
	...
	#update(thing) {
		...
		if (removed.length) {
			...
			this._server.mdns.respond(removed.map((record) => {
				...
				if (err) {
					report(thing, 'Could not withdraw the old records for ' + thing, err);
				}
			});
		}
	...
	#announce(thing, count = 1) {
		...
		thing.emit('announcing', JSON.parse(JSON.stringify(packet)));

		this._server.mdns.respond(packet, (err) => {
			if (err) {
				report(thing, 'Could not announce ' + thing, err);
			} else if (! thing.published) {
				thing._activated = true;
			...
			if (! thing._destroyed) {
				thing._announceTimer = setTimeout(this.#announce.bind(this, thing, count + 1), count * REANNOUNCE_FACTOR * 1000);
			}
		});
	}
	...
	#teardown(things) {
		...
		this._server.mdns.respond(records, (err) => {
			things.forEach((thing) => {
				if (err) {
					report(thing, 'Could not say goodbye for ' + thing, err);
				}
