...
export default class Registry {
	...
	publish(resource, probe = false) {
		...
		// hold it before starting it, so that unpublishing within the same tick
		// can still find it and call the start off
		this.#publications.set(resource, publication);

		publication._startTimer = setTimeout(() => {
	...
	// forget a publication, including one that has not started yet
	#release(publication) {
		clearTimeout(publication._startTimer);
	...
	#start(publication, probe = false) {
		// a publication we have already let go of is not ours to start
		if (publication._activated || ! this.#publications.has(publication.resource)) {
			return;
		}
	...
	#stop(publication, cb) {
		if (! publication._activated) {
			// nothing has gone out yet, so there is nothing to say goodbye to -
			// but letting it go is what calls off a start that has not happened
			this.#release(publication);
