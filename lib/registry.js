...
export default class Registry {
	...
	_start(publication, probe = false) {
		...
		if (probe && publication.resource instanceof Service) {
			...
			.then((exists) => {
				if (exists) {
					this._stop(publication);
					publication.emit('error', new Error('Service name is already in use on the network'));
					return;
				}
