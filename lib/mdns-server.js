...
export default class Server extends EventEmitter {
	...
	_respondToQuery(query) {
		query.questions.forEach((question) => {
			...
			let answers = type === 'ANY'
				? Object.keys(this.registry).map(this._recordsFor.bind(this, name)).flat(1)
			...
			this.mdns.respond({ answers, additionals }, (err) => {
				if (! err) {
					return;
				}

				// A response we could not send costs this querier an answer and
				// nothing else. It will ask again, and everything we publish
				// carries on regardless, so there is nothing here worth taking
				// the host process down over.
				console.warn('Could not answer a query for "' + name + '":', err.message);

				// An "error" event with nobody listening throws, which is the
				// very thing we are here to stop doing.
	...
	_recordsFor(name, type) {
		...
		.filter((record) => {
			let _name = ~ name.indexOf('.') ? record.name : record.name.split('.')[0];
			return dnsEqual(_name, name);
		});
	}

	#isDuplicateRecord(a, b) {
		return a.type === b.type && a.name === b.name && isDeepStrictEqual(a.data, b.data);
	}
