...
export default class Service extends EventEmitter {
	...
	toObject() {
		// txt is a proxy, and a proxy cannot be structuredClone'd - copy the pairs out
		return { fqdn: this.fqdn, ...this.#props, txt: this.#props.txt && { ...this.#props.txt }, host: this.host };
	}
	...
	[util.inspect.custom](depth, options, inspect) {
		if (depth <= 0) {
			return options.stylize('[' + this[Symbol.toStringTag] + ']', 'special');
		}

		return '<' + options.stylize(this[Symbol.toStringTag], 'special') + '> ' + inspect({ fqdn: this.fqdn, ...this.#props, addresses: this.addresses, host: this.host }, { ...options, depth: options.depth - 1 });
	}
