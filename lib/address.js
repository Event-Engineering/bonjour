...
export default class Address extends EventEmitter {
	...
	[util.inspect.custom](depth, options, inspect) {
		...
		return '<' + options.stylize(this[Symbol.toStringTag], 'special') + '> ' + inspect({ ...this.#props, addresses: this.addresses, name: this.name }, { ...options, depth: options.depth - 1 });
	}

	_records() {
		return addressRecords(this.name, this.addresses);
	}
}
