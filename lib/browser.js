...
var TLD = '.local'
var WILDCARD = '_services._dns-sd._udp'
...
function Browser (mdns, opts, onup) {
  ...
  this._domain = opts.domain || TLD;

  if ( ! this._domain.endsWith(TLD)) {
    this._domain += TLD;
  }
  if ( ! this._domain.startsWith('.')) {
    this._domain = '.' + this._domain;
  }

  if (!opts || !opts.type) {
    this._name = WILDCARD + this._domain;
    this._wildcard = true
  } else {
    this._name = serviceName.stringify(opts.type, opts.protocol || 'tcp') + this._domain
