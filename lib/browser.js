...
import { default as Service, serviceQuery as wildcardService } from './service.js';
...
export default class Browser extends EventEmitter {
	...
	get servicesExport() {
		return this._services.map(s => s.toObject());
	}
	...
	constructor(mdns, opts, onup, ondown) {
		...
		if (! opts || ! opts.type) {
			...
			let service = Service._fromObject(opts);
	...
	start() {
		...
		this._onresponse = (packet, rinfo) => {
			...
			Object.keys(nameMap).forEach((name) => {
				...
				let matches = this.#buildServicesFor(name, packet, rinfo);
	...
	update() {
		this.#query(this._name);
	}

	#query(name) {
		this._mdns.query(name, 'PTR', (err) => {
			if (err) {
				// a query nobody heard finds nothing, which otherwise looks the same as nothing being there
				report(this, 'Could not ask for "' + name + '"', err);
			}
		});
	}
	...
	#buildServicesFor(name, packet, referer) {
		...
		.map((ptr) => {
			...
			let txtR = ptr.subRecords.find(r => r.type === 'TXT');

			if (! srvR) {
				return;
			}

			let service = Service.fromFqdn(srvR.name, { ...srvR.data, referer, txtSettings: this._opts.txt });
