...
export default class ServiceObj {
	...
	set txt(value) {
		this.#props.rawTxt = Array.isArray(value) ? value.map(v => this.#dnsTxt.encode(v)) : this.#dnsTxt.encode(value);
	...
	set rawTxt(value) {
		...
		this.#props.txt = Array.isArray(value) ? value.map(v => this.#dnsTxt.decode(v)) : this.#dnsTxt.decode(value);
	}
