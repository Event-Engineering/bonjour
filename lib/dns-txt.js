const EQUALS = '='.charCodeAt(0);
const MAX_CHARACTER_STRING = 255;

// A key is printable US-ASCII bar the "=" that would end it, per RFC-6763
// section 6.4. A byte we cannot read as such arrives here as U+FFFD, which
// fails the same test.
const PRINTABLE_KEY = /^[\x20-\x3c\x3e-\x7e]+$/;

/**
 * The key/value pairs of a TXT record, per RFC-6763 section 6.
 *
 * A TXT record is a sequence of character-strings, each one a "key=value"
 * pair. The length prefixes that separate them belong to the DNS wire format
 * and are written by dns-packet, so everything here works in bare pairs: a
 * record is an array of buffers, one per pair, and the object it decodes to is
 * flat.
 */
export default class {
	#opts;

	get binary() {
		return !! this.#opts?.binary;
	}

	constructor(opts) {
		this.#opts = opts || {};
	}

	/**
	 * Encode a key/value object as the character-strings of a TXT record.
	 *
	 * A value of true is a valueless attribute, written as a bare key with no
	 * "=" at all. An empty object still has to produce one empty
	 * character-string, as a TXT record may not be empty.
	 */
	encode(data) {
		let pairs = Object.entries(data || {})
		.map(([ key, value ]) => {
			if (value === true) {
				return Buffer.from(key);
			}

			let prefix = Buffer.from(key + '=');

			return Buffer.concat(Buffer.isBuffer(value) ? [ prefix, value ] : [ prefix, Buffer.from(String(value)) ]);
		});

		pairs = pairs.filter((pair) => {
			if (pair.length <= MAX_CHARACTER_STRING) {
				return true;
			}

			console.warn('Ignoring "' + pair.subarray(0, 20).toString() + '..." in a TXT record: a key/value pair must encode to ' + MAX_CHARACTER_STRING + ' bytes or fewer, this one is ' + pair.length);

			return false;
		});

		return pairs.length ? pairs : [ Buffer.alloc(0) ];
	}

	/**
	 * Decode the character-strings of a TXT record into a flat object.
	 *
	 * Anything the specification tells us to disregard - an empty pair, a
	 * zero-length key, a key we have already seen - is skipped rather than
	 * rejected, so one bad pair cannot cost us the rest of the record.
	 */
	decode(rawTxt) {
		let data = {};

		(Array.isArray(rawTxt) ? rawTxt : [ rawTxt ])
		.forEach((pair) => {
			if (! pair?.length) {
				return; // the empty character-string of an empty TXT record
			}

			let index = pair.indexOf(EQUALS);

			if (index === 0) {
				console.warn('Ignoring "' + pair.subarray(0, 20).toString() + '" in a TXT record: a key/value pair must have a key');

				return;
			}

			let key = (index === - 1 ? pair : pair.subarray(0, index)).toString().toLowerCase();

			if (! PRINTABLE_KEY.test(key)) {
				console.warn('Ignoring "' + key.substring(0, 20) + '" in a TXT record: a key must be printable US-ASCII');

				return;
			}

			if (key in data) {
				return; // a repeated key keeps its first value, silently, as the specification asks
			}

			if (index === - 1) {
				data[key] = true; // a valueless attribute, present but with nothing to say
				return;
			}

			let value = pair.subarray(index + 1);

			data[key] = this.binary ? value : this.#cast(value.toString());
		});

		return data;
	}

	/**
	 * Read a value back as the type it was written as, where it can be written
	 * out again as the very same string.
	 *
	 * A TXT value is just text, so "3000" and "3000 widgets" are the same sort
	 * of thing to the wire and only one of them is a number. Encoding the
	 * candidate again and insisting on the original is what tells them apart,
	 * and it keeps hold of everything a plain Number() would quietly spoil:
	 * "007" and "1.10" would come back shortened, "1e3" and "0x10" rewritten,
	 * " 12" trimmed, and anything past the safe integers rounded.
	 *
	 * Numbers are all we read for now. Anything we cannot place is left as the
	 * string it arrived as.
	 */
	#cast(value) {
		let number = Number(value);

		if (! Number.isFinite(number) || String(number) !== value) {
			return value;
		}

		return number;
	}
}
