export default class {
	...
	decode(txtBuffer, offset = 0, length = null) {
		...
		while (offset < length) {
			offset += txtBuffer.readUInt8(offset) + 1;
		}

		if (offset !== length) {
			// non-RFC-6763 compliant format. Assume RFC-1464.
			this.decode.bytes = length - oldOffset;
			txtBuffer = txtBuffer.slice(oldOffset);

			if (this.binary) {
				return txtBuffer;
			}

			txtBuffer = txtBuffer.toString();
			let index = txtBuffer.indexOf('=');

			return Object.fromEntries([[txtBuffer.substring(0, index), txtBuffer.substring(index + 1)]]);
		}
